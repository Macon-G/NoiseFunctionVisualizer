...
namespace Noise {
	Perlin::Perlin(uint32_t seed) : seed_(seed) {}

	float Perlin::Sample(float x) const {
		// Find the two lattice points around the sample.
		const int32_t x0 = static_cast<int32_t>(std::floor(x));
		const int32_t x1 = x0 + 1;

		// Measure the sample's offset from each lattice point.
		const float dx0 = x - static_cast<float>(x0);
		const float dx1 = dx0 - 1.0f;

		// Pick a deterministic unit gradient for each lattice point.
		const float g0 = (Hash::Hash1D(x0, seed_) & 1u) ? 1.0f : -1.0f;
		const float g1 = (Hash::Hash1D(x1, seed_) & 1u) ? 1.0f : -1.0f;

		// Smooth the interpolation amount and blend the gradient contributions.
		const float t = Math::Smootherstep(dx0);
		return Math::Lerp(t, g0 * dx0, g1 * dx1) * 2.0f;	// Adjust output range to [-1, 1]
	}

	float Perlin::Sample(float x, float y) const {
